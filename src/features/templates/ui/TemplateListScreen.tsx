...
export default function TemplateListScreen() {
  ...
  const handleCreate = () => {
    ...
    const created = createTemplate({
      ...
      scope: newScope,
      content: { note: newContentText.trim() },
    ...
    setNewDesc('');
    setNewContentText('');
    Alert.alert('Template Salvo', 'O novo modelo clínico está disponível para uso imediato.');
  };

  const handleDuplicate = (item: ClinicalTemplate) => {
    const copy = duplicateTemplate(item);
    setTemplates(prev => [copy, ...prev]);
    Alert.alert('Template Duplicado', `Criada cópia de "${item.name}".`);
  };

  const handleDelete = (id: string) => {
    Alert.alert('Excluir Template', 'Deseja remover este modelo?', [
      { text: 'Cancelar', style: 'cancel' },
      {
        text: 'Excluir',
        style: 'destructive',
        onPress: () => setTemplates(prev => prev.filter(t => t.id !== id))
      }
    ]);
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1, gap: 2 }}>
          <Txt style={styles.headerTitle}>Templates Clínicos</Txt>
          <Txt muted style={{ fontSize: 13 }}>Padronização ágil e segura de condutas</Txt>
        </View>
        <IconButton 
          icon={Plus} 
          label="Novo Template" 
        ...
        renderItem={({ item }) => (
          <Card style={styles.templateCard}>
            <View style={s.between}>
              <Badge tone={item.scope === 'organization' ? 'green' : 'neutral'}>
                {item.scope === 'organization' ? 'Compartilhado com Clínica' : 'Uso Pessoal'}
              </Badge>
              <View style={s.row}>
                <IconButton 
                  icon={Copy} 
...
const styles = StyleSheet.create({
  ...
  templateCard: { gap: 10 },
  cardName: { fontFamily: fonts.semibold, fontSize: 16, color: c.text },
  cardFooter: {
    flexDirection: 'row',
