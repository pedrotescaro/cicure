    ...
    <>
      <Tabs
        ...
        screenLayout={({ children, route }) => (
          <BlurTargetView ref={getBlurTarget(route.key)} style={styles.scene}>
            {children}
          </BlurTargetView>
        )}
        screenOptions={{ headerShown: false, animation: 'none', tabBarStyle: { position: 'absolute' } }}
      >
      ...
      {!presentation && (
        ...
        >
          <Plus size={24} color={colors.surface} strokeWidth={2.4} />
        </Pressable>
      )}
    </>
  );
...
const styles = StyleSheet.create({
  scene: { flex: 1, backgroundColor: colors.bg },
  fab: { position: 'absolute', zIndex: 7, width: metrics.fabSize, height: metrics.fabSize,
    borderRadius: metrics.fabSize / 2, alignItems: 'center', justifyContent: 'center',
    shadowColor: colors.dark, shadowOpacity: 0.18, shadowRadius: 14,
